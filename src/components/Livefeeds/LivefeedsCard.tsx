...
const LivefeedsCard: React.FC<ILivefeedsCardProps> = ({
  feed,
  onFollow,
  onJoin,
}) => {
  return (
    <div className="livefeed-card">
      ...
      <div className="chatters">
        <p>@{feed.username}</p>
        {feed.cooldown !== undefined && (
          <p className="cooldown">Cooldown: {feed.cooldown} min</p>
        )}
      </div>
      <div className="actions">
        <button className="follow-btn" onClick={() => onFollow(feed.username)}>
          Follow
        </button>
        <p className="followers">{feed.followers} Followers</p>
        <button className="join-btn" onClick={() => onJoin(feed.title)}>
          Join
        </button>
      </div>
    </div>
